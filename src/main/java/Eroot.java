...
class Eroot extends Production {
    ...
    Vertex apply(Vertex T) {
        ...
        divider = T.m_a[0][0];
        for (int i = 0; i < 3; i++) {
            T.m_a[0][i] = T.m_a[0][i] / divider;
        }
        T.m_b[0] /= divider;

        multiplier = T.m_a[1][0] / T.m_a[0][0];
        for (int i = 0; i < 3; i++) {
            T.m_a[1][i] = T.m_a[1][i] - multiplier * T.m_a[0][i];
        }
        T.m_b[1] = T.m_b[1] - multiplier * T.m_b[0];

        multiplier = T.m_a[2][0] / T.m_a[0][0];
        for (int i = 0; i < 3; i++) {
            T.m_a[2][i] = T.m_a[2][i] - multiplier * T.m_a[0][i];
        }
        T.m_b[2] = T.m_b[2] - multiplier * T.m_b[0];

        divider = T.m_a[1][1];
        for (int i = 1; i < 3; i++) {
            T.m_a[1][i] = T.m_a[1][i] / divider;
        }
        T.m_b[1] = T.m_b[1] / divider;

        multiplier = T.m_a[2][1];
        for (int i = 1; i < 3; i++) {
            T.m_a[2][i] -= multiplier * T.m_a[1][i];
        }
        T.m_b[2] = T.m_b[2] - multiplier * T.m_b[1];
